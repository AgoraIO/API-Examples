---
name: add-new-case
description: Add a new API example case to APIExample (full demo). Use when a PRD requires a new Agora RTC feature demonstration using Java/Kotlin + XML layouts.
---
...
- Permissions:        # CAMERA + RECORD_AUDIO (video) or RECORD_AUDIO (audio-only)
- Reference case:     # Closest existing case file, e.g. JoinChannelVideo.java
...
- Add `@Example` annotation — all four fields are required

```java
/**
 * This demo demonstrates how to use [describe the API feature here].
 *
 * Key APIs used:
 * - [list the primary Agora APIs, e.g. RtcEngine.takeSnapshot()]
 *
 * @see <a href="https://docs.agora.io/en/">Agora API Reference</a>
 */
@Example(
    index    = 10,                                        // unique within the group
    group    = ADVANCED,                                  // BASIC or ADVANCED
    name     = R.string.item_your_case_name,              // list display name
    actionId = R.id.action_mainFragment_to_yourCaseName,  // nav action ID
    tipsId   = R.string.your_case_name_tips               // description shown on the case screen
)
public class YourCaseName extends BaseFragment implements View.OnClickListener {
    ...
    private final IRtcEngineEventHandler iRtcEngineEventHandler = new IRtcEngineEventHandler() {
        ...
        public void onUserJoined(int uid, int elapsed) {
            ...
            runOnUIThread(() -> { /* add remote view */ });
        }

        @Override
        public void onUserOffline(int uid, int reason) {
            Log.i(TAG, String.format("user %d offline, reason %d", uid, reason));
            runOnUIThread(() -> { /* remove remote view */ });
        }
    };
}
```
...
Typical structure:
- Top: channel name input + Join/Leave button
- Middle: `VideoReportLayout` for video, or audio widgets
- Bottom: feature control buttons
...
<!-- description shown on the case screen -->
<string name="your_case_name_tips">This example demonstrates how to use XXX API.</string>
```
...
The action `android:id` must exactly match `actionId` in `@Example`.
...
- [ ] Case appears in the correct group (BASIC / ADVANCED)
- [ ] Tapping the case navigates to the case screen
- [ ] `onJoinChannelSuccess` fires after joining
- [ ] No engine leak after leaving the screen (check Logcat)
...
| Audio/video leak after switching cases | `leaveChannel()` or `RtcEngine.destroy()` missing in `onDestroy()` |
