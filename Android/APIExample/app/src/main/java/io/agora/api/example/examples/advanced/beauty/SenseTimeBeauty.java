...
public class SenseTimeBeauty extends BaseFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        channelId = arguments.getString(getString(R.string.key_channel_name));
        initVideoView();
        if (!initRtcEngine()) {
            return;
        }
        ...
        try {
            mRtcEngineEventHandler = new IRtcEngineEventHandler() {
                ...
                public void onError(int err) {
                    super.onError(err);
                    runOnUIThread(() -> showLongToast(String.format(Locale.US, "msg:%s, code:%d", RtcEngine.getErrorDescription(err), err)));
                }
                ...
                public void onLocalAudioStats(LocalAudioStats stats) {
                    ...
                    runOnUIThread(() -> {
                        if (mLocalVideoLayout != null) {
                            mLocalVideoLayout.setLocalAudioStats(stats);
                        }
                    });
                }
                ...
                public void onLocalVideoStats(Constants.VideoSourceType source, LocalVideoStats stats) {
                    ...
                    runOnUIThread(() -> {
                        if (mLocalVideoLayout != null) {
                            mLocalVideoLayout.setLocalVideoStats(stats);
                        }
                    });
                }
                ...
                public void onRemoteAudioStats(RemoteAudioStats stats) {
                    ...
                    runOnUIThread(() -> {
                        if (mRemoteVideoLayout != null) {
                            mRemoteVideoLayout.setRemoteAudioStats(stats);
                        }
                    });
                }
                ...
                public void onRemoteVideoStats(RemoteVideoStats stats) {
                    ...
                    runOnUIThread(() -> {
                        if (mRemoteVideoLayout != null) {
                            mRemoteVideoLayout.setRemoteVideoStats(stats);
                        }
                    });
                }
            };
            ...
            config.mAreaCode = ((MainApplication) requireActivity().getApplication())
                    .getGlobalSettings().getAreaCode();
            rtcEngine = RtcEngine.create(config);
            rtcEngine.setParameters("{\"rtc.report_app_scenario\":{\"appScenario\":100,\"serviceType\":11,\"appVersion\":\""
                    + RtcEngine.getSdkVersion() + "\"}}");
            LocalAccessPointConfiguration privateCloudConfig = ((MainApplication) requireActivity().getApplication())
                    .getGlobalSettings().getPrivateCloudConfig();
            if (privateCloudConfig != null) {
                rtcEngine.setLocalAccessPoint(privateCloudConfig);
            }
    ...
    private void joinChannel() {
        ...
        TokenUtils.gen(requireActivity(), channelId, uid, token -> {
            ...
            if (!isAdded() || engine == null) {
                return;
            }
            int ret = engine.joinChannel(token, channelId, uid, options);
