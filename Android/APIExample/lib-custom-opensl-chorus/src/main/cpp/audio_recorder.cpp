...
void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
  ...
  sample_buf *freeBuf = NULL;

  if (freeQueue_->front(&freeBuf)) {
    recQueue_->push(freeBuf);
    SLresult result = (*bq)->Enqueue(bq, freeBuf->buf_, freeBuf->cap_);
    SLASSERT(result);
    freeQueue_->pop();
  }
  sample_buf *recBuf = NULL;
  if (recQueue_->front(&recBuf)) {
    callback_(ctx_, ENGINE_SERVICE_MSG_RECORDED_AUDIO_AVAILABLE, recBuf);
    freeQueue_->push(recBuf);
    recQueue_->pop();
  }
}
