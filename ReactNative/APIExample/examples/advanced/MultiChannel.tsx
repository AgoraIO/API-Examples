...
import { View, TextInput, PermissionsAndroid, StyleSheet, Button, Platform } from 'react-native';

import RtcEngine, { RtcChannel, RtcLocalView, RtcRemoteView } from 'react-native-agora';
import { ChannelProfile, ClientRole, ChannelMediaOptions } from 'react-native-agora/lib/Types';

const config = require('../../agora.config');
...
export default class MultiChannel extends Component<{}, State, any> {
    ...
    _joinChannel0 = async () => {
        ...
        await this._channel0.joinChannel(null, null, 0, new ChannelMediaOptions(true, true));
    }
    ...
    _joinChannel1 = async () => {
        ...
        await this._channel1.joinChannel(null, null, 0, new ChannelMediaOptions(true, true));
    }
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.float}>
                    ...
                    <Button
                        onPress={() => { this.setState({ renderChannelId: channelId0 }) }}
                        title={`Render ${channelId0}`} />
                    <Button
                        onPress={this._publishChannel1}
                        title={`Pulish ${channelId1}`} />
                    <Button
                        onPress={() => { this.setState({ renderChannelId: channelId1 }) }}
