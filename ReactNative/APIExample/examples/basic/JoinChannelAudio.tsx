...
import { ChannelProfile, ClientRole } from 'react-native-agora/lib/Types';
...
export default class JoinChannelAudio extends Component<{}, State, any> {
    ...
    _joinChannel = async () => {
        ...
        await this._engine?.joinChannel(config.token, this.state.channelId, null, 0);
    }
    ...
    _switchSpeakerphone = async () => {
        ...
        await this._engine?.setEnableSpeakerphone(!enableSpeakerphone).then(() => {
    ...
    render() {
        const { channelId, isJoined, openMicrophone, enableSpeakerphone } = this.state;
        return (
