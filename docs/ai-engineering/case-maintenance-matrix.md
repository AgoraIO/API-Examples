...
This matrix is the planning artifact for cross-platform API Example parity work. It does not replace platform `AGENTS.md`, project `ARCHITECTURE.md`, or project `.agents/skills/*`; every row must be verified against the target project before implementation.
...
Seeded from the project `ARCHITECTURE.md` indexes and source tree, then re-verified against `dev/4.7.0` (`ee77cc85`). Paths are relative to the projects listed in Platform Units.

Inventory scope: rows were derived from the `Android/APIExample/` case index and its cross-platform counterparts. Cases that exist only outside Android full are **not inventoried yet** — including `ARKit`, `FaceCapture`, `MutliCamera`/`MultiCameraSourece`, `RawMediaData`, `CustomVideoSourceMediaIO`, `VideoChat`, and the Windows-only dialogs (`AudioProfile`, `AudioVolume`, `RegionConn`, `ReportInCall`, `RTMPinject`, `MultiVideoSource`). Their absence from this matrix is a documented inventory gap, not a coverage claim.
...
| Voice beautifier and effects | Full RTC or audio SDK | `setVoiceBeautifierPreset`, `setAudioEffectPreset`, `setVoiceConversionPreset` | `DONE(advanced/VoiceEffects.java)` | `DONE(advanced/VoiceEffects.java)` | `DONE(samples/VoiceEffects.kt)` | `DONE(Advanced/VoiceChanger/)` | `DONE(Advanced/VoiceChanger/)` | `DONE(Advanced/VoiceChanger/)` | `DONE(Advanced/VoiceChanger/)` | `DONE(Advanced/VoiceChanger/)` | `DONE(Advanced/BeautyAudio/)` | Full coverage, verified by `setVoiceBeautifierPreset`. Windows bundles it with video beauty in `BeautyAudio`. |
| Audio waveform | Full RTC or audio SDK | `enableAudioVolumeIndication`, audio frame observer waveform rendering | `DONE(audio/AudioWaveform.java)` | `DONE(audio/AudioWaveform.java)` | `MISSING` | `DONE(Advanced/AudioWaveform/)` | `DONE(Advanced/AudioWaveform/)` | `MISSING` | `MISSING` | `MISSING` | `MISSING` | Windows `AudioVolume` adjusts signal volume and is not a waveform case. |
| Third-party player audio routing | Full RTC or audio SDK | third-party player playback combined with the SDK audio route | `DONE(audio/AudioRouterPlayer.java)` | `MISSING` | `MISSING` | `DONE(Advanced/AudioRouterPlayer/)` | `MISSING` | `MISSING` | `MISSING` | `MISSING` | `MISSING` | Android ships Exo/Ijk/Native variants. Confirm the third-party player dependency is available before porting. |
| Agora beauty (video effect object) | Full RTC | `createVideoEffectObject`, `setVideoEffectStringParam`, `setVideoEffectFloatParam` | `DONE(advanced/AgoraBeauty.java)` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/AgoraBeauty/)` | `MISSING` | `MISSING` | `N/A(audio SDK)` | `DONE(Advanced/AgoraBeauty/)` | `DONE(Advanced/Beauty2.0/)` | Distinct from the built-in `setBeautyEffectOptions` video-enhancement case. Verified by `createVideoEffectObject`. |
| Simulcast | Full RTC | `setSimulcastConfig` | `DONE(advanced/Simulcast.java)` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/Simulcast/)` | `DONE(Advanced/Simulcast/)` | `DONE(Advanced/Simulcast/)` | `N/A(audio SDK)` | `DONE(Advanced/Simulcast/)` | `DONE(Advanced/Simulcast/)` | Android Compose is the only full-RTC gap. Verified by `setSimulcastConfig`. |
| Multiple custom video tracks | Full RTC | `createCustomVideoTrack`, `joinChannelEx`, `pushExternalVideoFrameById` | `DONE(advanced/MultiVideoSourceTracks.java)` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/CustomVideoSourcePushMulti/)` | `MISSING` | `MISSING` | `N/A(audio SDK)` | `DONE(Advanced/CustomVideoSourcePushMulti/)` | `DONE(Advanced/MultiVideoSourceTracks/)` | Case names differ per platform; correspondence verified by `createCustomVideoTrack`. |
| Custom remote video render | Full RTC | remote render path via `registerVideoFrameObserver` or `setVideoFrameDelegate` | `DONE(advanced/CustomRemoteVideoRender.java)` | `N/A(audio SDK)` | `DONE(samples/CustomVideoRender.kt)` | `DONE(Advanced/CustomVideoRender/)` | `MISSING` | `DONE(Advanced/CustomVideoRender/)` | `N/A(audio SDK)` | `DONE(Advanced/CustomVideoRender/)` | `MISSING` | SwiftUI has no CustomVideoRender case. Windows `OriginalVideo` is raw video data, tracked in its own row. |
| Transparent background rendering | Full RTC | alpha mask and alpha stitch mode with encoded alpha | `DONE(advanced/TransparentRendering.java)` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/TransparentRender/)` | `MISSING` | `MISSING` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/TransparentBg/)` | macOS uses alpha options only inside `VideoProcess`/`AgoraBeauty`, not in a transparent-rendering case. |
| Quick channel switch | Full RTC | rapid `joinChannel` / `leaveChannel` switching across channels | `DONE(advanced/VideoQuickSwitch.java)` | `N/A(audio SDK)` | `MISSING` | `DONE(Advanced/QuickSwitchChannel/)` | `DONE(Advanced/QuickSwitchChannel/)` | `MISSING` | `N/A(audio SDK)` | `DONE(Advanced/QuickSwitchChannel/)` | `MISSING` | Correspondence is by scenario and case name; this feature has no single distinctive API to verify against. |
| URL live stream player | Full RTC | RTE player `openWithUrl`, `startWithUrl` | `DONE(advanced/UrlLiveStream.java)` | `N/A(audio SDK)` | `MISSING` | `UNKNOWN` | `MISSING` | `MISSING` | `N/A(audio SDK)` | `UNKNOWN` | `UNKNOWN` | iOS UIKit, macOS, and Windows ship an `RtePlayer` case that was not confirmed to demonstrate the same scenario. Run `query-cases` before treating these as gaps. |
| KTV copyright music | None (docs link) | none — the case only opens the Agora documentation page | `DONE(advanced/KtvCopyrightMusic.java)` | `N/A(docs-link case)` | `N/A(docs-link case)` | `DONE(Advanced/KtvCopyrightMusic/)` | `DONE(Advanced/KtvCopyrightMusic/)` | `N/A(docs-link case)` | `N/A(docs-link case)` | `N/A(docs-link case)` | `N/A(docs-link case)` | All three implementations only open a docs URL (Android `WebView`, iOS/SwiftUI hand off to the browser). No SDK surface to port, so absences are not gaps. |
...
| Audio waveform | Android Compose, iOS Objective-C, iOS audio, macOS, Windows | Medium — audio-visualisation gap on five units |
| Simulcast | Android Compose | Medium — only full-RTC gap for a shipped 4.x feature |
| Custom remote video render | iOS SwiftUI, Windows | Medium — full-RTC variant gap |
| Multiple custom video tracks | Android Compose, iOS SwiftUI, iOS Objective-C | Medium — multi-track gap |
| Transparent background rendering | Android Compose, iOS SwiftUI, iOS Objective-C, macOS | Low — niche rendering feature |
| Quick channel switch | Android Compose, iOS Objective-C, Windows | Low — switching-performance demo gap |
| Agora beauty (video effect object) | Android Compose, iOS SwiftUI, iOS Objective-C | Low — beauty v2 gap |
| Third-party player audio routing | Android audio, Android Compose, iOS SwiftUI, iOS Objective-C, iOS audio, macOS, Windows | Low — depends on third-party player dependencies |
