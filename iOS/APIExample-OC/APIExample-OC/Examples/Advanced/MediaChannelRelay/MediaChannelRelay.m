...
@implementation MediaChannelRelayEntry
...
@interface MediaChannelRelay ()<AgoraRtcEngineDelegate>
@property (weak, nonatomic) IBOutlet UIView *containerView;
@property (weak, nonatomic) IBOutlet UITextField *relayChannelField;
...
- (void)setIsRelaying:(BOOL)isRelaying {
    _isRelaying = isRelaying;
    [self.relayChannelField setEnabled:!isRelaying];
}
...
- (IBAction)doRelay:(UIButton *)sender {
    NSString *destinationChannelName = self.relayChannelField.text;
    if (destinationChannelName.length <= 0) {
        [self showAlertWithTitle:@"Destination channel name is empty"];
        return;
    }
...
- (IBAction)doPause:(id)sender {
    [self.agoraKit pauseAllChannelMediaRelay];
}
- (IBAction)doResume:(id)sender {
    [self.agoraKit resumeAllChannelMediaRelay];
}
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine channelMediaRelayStateDidChange:(AgoraChannelMediaRelayState)state error:(AgoraChannelMediaRelayError)error {
    [LogUtil log:[NSString stringWithFormat:@"channelMediaRelayStateDidChange: %ld error: %ld", state, error]];
    switch (state) {
        ...
        case AgoraChannelMediaRelayStateFailure:
            self.isRelaying = NO;
            [self showAlertWithTitle:[NSString stringWithFormat:@"Media Relay Failed: %ld", error]];
            break;
