...
@interface PictureInPictureEntry ()
@property (weak, nonatomic) IBOutlet UITextField *textField;

@end

@implementation PictureInPictureEntry
...
- (IBAction)onClickJoinButton:(id)sender {
    ...
    UIStoryboard *storyBoard = [UIStoryboard storyboardWithName:@"PictureInPicture" bundle:nil];
    BaseViewController *newViewController = [storyBoard instantiateViewControllerWithIdentifier:@"PictureInPicture"];
...
@interface PictureInPicture ()<AgoraRtcEngineDelegate, AgoraVideoFrameDelegate, AVPictureInPictureControllerDelegate>
@property (weak, nonatomic) IBOutlet UIView *containerView;
@property (nonatomic, strong)VideoView *localView;
@property (nonatomic, strong)SampleBufferDisplayView *remoteView;
...
- (SampleBufferDisplayView *)remoteView {
    if (_remoteView == nil) {
        _remoteView = (SampleBufferDisplayView *)[NSBundle loadViewFormNib:@"VideoViewSampleBufferDisplayView"];
    }
    return _remoteView;
}
...
- (IBAction)onPIP:(UIButton *)sender {
    if (self.pipController.pipController) {
        ...
        [self showAlertWithTitle:@"PIP Support iOS 15+".localized];
    }
}

- (void)viewDidDisappear:(BOOL)animated {
    [super viewDidDisappear:animated];
    if (self.pipController.pipController && self.pipController.pipController.isPictureInPictureActive) {
...
- (BOOL)onRenderVideoFrame:(AgoraOutputVideoFrame *)videoFrame uid:(NSUInteger)uid channelId:(NSString *)channelId {
    [self.remoteView.videoView renderVideoPixelBuffer:videoFrame];
    return YES;
}
...
- (void)pictureInPictureControllerWillStartPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
    [LogUtil log:@"pictureInPictureControllerWillStartPictureInPicture"];
}

- (void)pictureInPictureControllerDidStartPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
    [LogUtil log:@"pictureInPictureControllerDidStartPictureInPicture"];
}

- (void)pictureInPictureController:(AVPictureInPictureController *)pictureInPictureController failedToStartPictureInPictureWithError:(NSError *)error {
    [LogUtil log:@"failedToStartPictureInPictureWithError"];
}

- (void)pictureInPictureControllerWillStopPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
    [LogUtil log:@"pictureInPictureControllerWillStopPictureInPicture"];
}

- (void)pictureInPictureControllerDidStopPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
    [LogUtil log:@"pictureInPictureControllerDidStopPictureInPicture"];
}
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didJoinedOfUid:(NSUInteger)uid elapsed:(NSInteger)elapsed {
    ...
    [self.remoteView.videoView reset];
}
