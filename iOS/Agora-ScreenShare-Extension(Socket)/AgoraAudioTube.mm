...
PushAudioFrame pushAudioFrame;

static NSObject *lock = [[NSObject alloc] init];

@implementation AgoraAudioTube

+ (void)registerAudioPreprocessing:(PushAudioFrame)callback {
    pushAudioFrame = callback;
}
...
+ (void)pushAudioCMSampleBuffer:(CMSampleBufferRef _Nonnull)sampleBuffer resampleRate:(NSUInteger)resampleRate type:(AudioType)type; {
    @synchronized (lock) {
        [self privatePushAudioCMSampleBuffer:sampleBuffer
                                resampleRate:resampleRate
                                        type:type];
    }
}

+ (void)privatePushAudioCMSampleBuffer:(CMSampleBufferRef _Nonnull)sampleBuffer resampleRate:(NSUInteger)resampleRate type:(AudioType)type {
    ...
    switch (type) {
        case AudioTypeApp: {
            ...
            if (pushAudioFrame) {
                pushAudioFrame((unsigned char *)pushBuffer, appAudioIndex * sizeof(int16_t));
            }
