...
void CAgoraMediaPlayer::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_ATTACH, m_btnAttchPlayer);
	DDX_Control(pDX, IDC_BUTTON_PUBLISH_VIDEO, m_btnPublishVideo);
	DDX_Control(pDX, IDC_BUTTON_PUBLISH_AUDIO, m_btnPublishAudio);
...
BEGIN_MESSAGE_MAP(CAgoraMediaPlayer, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_ATTACH, &CAgoraMediaPlayer::OnBnClickedButtonAttach)
...
void CAgoraMediaPlayer::OnBnClickedButtonAttach()
{
	if (!m_attach)
	{
...
// push audio button click handler.
void CAgoraMediaPlayer::OnBnClickedButtonPublishAudio()
{
	if (!m_publishAudio)
	{
